    ...
    <section className="card" style={{ display: "grid", gap: 12 }}>
      ...
      {showCoupons ? (
        <div className="card" style={{ display: "grid", gap: 10 }}>
          ...
          {coupons.length === 0 ? (
            <div className="task-meta">
              No coupons yet. Redeem a reward to add one here.
            </div>
          ) : (
            <ul
              style={{
            ...
            >
              {coupons
                ...
                .map((c) => (
                  <li
                    key={c.id}
                    className={`coupon coupon--held coupon--${String(
                      c.tier || "Small"
                    ).toLowerCase()}`}
                  >
            ...
            return (
              <div key={tier} className="card" style={{ display: "grid", gap: 10 }}>
                ...
                {list.map((r) => (
                  <div
                    key={r.key}
                    className={`coupon coupon--${r.tier.toLowerCase()}`}
                  >
                    <span className="fx-stars" />
                    <span className="fx-sheen" />
                    <span className="cut" aria-hidden="true" />
