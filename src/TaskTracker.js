...
/* tier -> css class for coupon color */
const tierClass = (tier) => {
  switch (tier) {
    case "Small": return "coupon--small";
    case "Medium": return "coupon--medium";
    case "Large": return "coupon--large";
    case "Special": return "coupon--special";
    default: return "";
  }
};
...
export default function TaskTracker(){
  ...
  /* checklists */
  const [lists,setLists]=useState(()=>load("checklists",[
  ...
  const [addCat,setAddCat]=useState("");     // "Items" placeholder
  const [addTime,setAddTime]=useState("");   // "Time" placeholder
  ...
  const updateEventTime=(id,newHHMM)=>setEvents(p=>p.map(e=>{
    if(e.id!==id) return e;
  ...
  function redeem(r){
    ...
    const coupon={ id:uid(), reward:r.key, label:r.label, cost:r.cost, date:new Date().toISOString() };
    setCoupons(prev=>[...prev,coupon]);
    setRedemptions(prev=>[...prev,{...coupon}]);
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} className="menu">
                {TABS.map(t=>(
                  <button key={t.key} className={`menu-item ${tab===t.key?"active":""}`}
        ...
        <section className="card" style={{ display:"grid", gap:12 }}>
          ...
          <div className="card" style={{display:"flex", gap:8, alignItems:"center", justifyContent:"space-between", flexWrap:"wrap"}}>
            <div style={{display:"flex", gap:8, alignItems:"center"}}>
              <button className="menu-item" onClick={()=>{ const d=new Date(dayDate); d.setDate(d.getDate()-1); setDayDate(d.toISOString().slice(0,10)); }}>◀</button>
          ...
          {showAdd && (
            <div className="card" style={{ display:"grid", gap:10 }}>
              ...
              <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
                ...
                <button className="btn" onClick={()=>{ const cat=addCat||"other"; const time=addTime||"08:00"; const title=addTitle||""; addEvent({title,timeHHMM:time,category:cat}); setShowAdd(false); setAddCat(""); setAddTime(""); setAddTitle(""); }}>Save</button>
              </div>
            </div>
          )}
              ...
              return (
                <div key={i} className={`month-cell ${inMonth?"":"month-fade"}`}>
                  ...
                  <div className="month-items">
                    {dayItems.slice(0,3).map(e=><div key={e.id} className={`month-chip ${e.done?"month-chip-done":""}`}>{new Date(e.whenISO).toTimeString().slice(0,5)} · {e.title}</div>)}
              ...
              {["Small","Medium","Large","Special"].map(tier=>{
                ...
                return (
                  <div key={tier} className="card" style={{ display:"grid", gap:10 }}>
                    ...
                    {list.map(r=>(
                      <div key={r.key} className={`coupon ${tierClass(r.tier)}`}>
                        {/* sparkle */}
                        <span className="fx-stars" aria-hidden="true"></span>
                        <span className="fx-sheen" aria-hidden="true"></span>
      ...
      {tab==="checklists" && (
        <section className="card" style={{ display:"grid", gap:12 }}>
          <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:6 }}>
            {lists.map(l=>(
              <button key={l.id} className={`menu-item ${activeListId===l.id?"active":""}`} onClick={()=>setActiveListId(l.id)} style={{ whiteSpace:"nowrap" }}>
                {l.name}
              </button>
            ))}
          </div>
